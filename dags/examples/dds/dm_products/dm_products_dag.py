...
def sprint5_example_dds_products_dag():
    ...
    def load_products():
        events_loader = ProductLoader(dwh_pg_connect, dwh_pg_connect, log)
        events_loader.load_product()

    # Подключаем задачу загрузки событий в цепочку зависимостей
    dm_products_task = load_products()

    # Теперь порядок выполнения операций следующий:
    dm_products_task
    

stg_bonus_system_ranks_dag = sprint5_example_dds_products_dag()
