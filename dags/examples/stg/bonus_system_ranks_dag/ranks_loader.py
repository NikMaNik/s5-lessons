...
class RanksOriginRepository:
    ...
    def list_users(self, user_threshold:int, limit: int) -> List[RankObjUser]:
        with self._db.client().cursor(row_factory = class_row(RankObjUser)) as cur:
            cur.execute(
                """
                    SELECT id, 
                            order_user_id
...
class RankDestRepository:
    ...
    def insert_user(self, conn: Connection, user: RankObjUser) -> None:
        with conn.cursor() as cur:
            cur.execute(
                """
                    ISERT INTO stg.bonussytem_users
                    (id, order_user_id)
                    VALUES 
